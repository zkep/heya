...
from heya.core.logging.logging import ErrorInfo
...
from heya.web.services.service import WebConvertService
...
__all__ = [
    ...
    "convert_wechat_to_pdf_with_error_handling",
    "merge_md_pdfs",
    "merge_wechat_pdfs",
...
async def convert_md_to_pdf_stream(
    md_files: list[str] | str,
    timeout: float,
    quality: int,
    lang: str,
) -> AsyncGenerator[StreamProgressResponse, None]:
    async for result in _markdown_handler.convert_stream(md_files, timeout, quality, lang):
        yield result
...
async def convert_wechat_to_pdf_stream(
    url: str,
    timeout: float,
    quality: int,
    lang: str,
) -> AsyncGenerator[StreamProgressResponse, None]:
    async for result in _wechat_handler.convert_stream(url, timeout, quality, lang):
        yield result


def convert_wechat_to_pdf_with_error_handling(
    url: str,
    timeout: float,
    quality: int,
    lang: str,
) -> HandlerResponse:
    return _wechat_handler.convert_with_error_handling(url, timeout, quality, lang)
