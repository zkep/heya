...
class WechatParser:
    ...
    @staticmethod
    def parse_article_list(url: str) -> list[WechatArticle]:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                page.goto(normalized_url, timeout=30000, wait_until="networkidle")
                page.wait_for_timeout(2000)

                if "/appmsgalbum" in normalized_url:
                    WechatParser._scroll_to_load_all_articles(page)

                articles = WechatParser._extract_articles_from_page(page)
                browser.close()
    ...
    @staticmethod
    def _scroll_to_load_all_articles(page: Any) -> None:
        last_height = 0
        max_scrolls = 10
        scroll_count = 0
        stable_count = 0
        ...
        try:
            cgi_data = page.evaluate("window.cgiData")
    ...
    @staticmethod
    def _extract_articles_from_page(page: Any) -> list[WechatArticle]:
        selector_strategies = [
    ...
    @staticmethod
    def _try_extract_by_profile_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("a.weui_media_appmsg")
            for idx, elem in enumerate(article_elements):
                title_elem = elem.query_selector("h4.weui_media_title")
                title = title_elem.inner_text() if title_elem else f"Article {idx + 1}"
    ...
    @staticmethod
    def _try_extract_by_js_title_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("a.js_title")
            for idx, elem in enumerate(article_elements):
                title = elem.inner_text() or f"Article {idx + 1}"
                url = elem.get_attribute("href")
                if url:
                    articles.append(WechatArticle(title=title, url=url, index=idx + 1))
        except Exception:
    ...
    @staticmethod
    def _try_extract_by_link_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("a[href*='/s/']")
            for idx, elem in enumerate(article_elements):
                title = elem.inner_text() or f"Article {idx + 1}"
                url = elem.get_attribute("href")
                if url and "/s/" in url:
    ...
    @staticmethod
    def _try_extract_by_album_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("div[data-sku*='appmsgalbum'] a")
            for idx, elem in enumerate(article_elements):
                url = elem.get_attribute("href")
                if url and "/s/" in url:
                    title = elem.inner_text() or f"Article {idx + 1}"
    ...
    @staticmethod
    def _try_extract_by_general_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("a")
            for idx, elem in enumerate(article_elements):
                url = elem.get_attribute("href")
                if url and "/s/" in url:
                    title = elem.inner_text() or f"Article {idx + 1}"
    ...
    @staticmethod
    def _try_extract_by_appmsg_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("div.appmsg_item a")
            for idx, elem in enumerate(article_elements):
                title_elem = elem.query_selector("h3.appmsg_title")
                title = title_elem.inner_text() if title_elem else f"Article {idx + 1}"
                url = elem.get_attribute("href")
                if url:
                    articles.append(WechatArticle(title=title, url=url, index=idx + 1))
    ...
    @staticmethod
    def _try_extract_by_appmsg_link(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("a.appmsg-link")
            for idx, elem in enumerate(article_elements):
                title = elem.inner_text() or f"Article {idx + 1}"
                url = elem.get_attribute("href")
                if url:
    ...
    @staticmethod
    def _try_extract_by_card_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("div.card-wrap a")
            for idx, elem in enumerate(article_elements):
                url = elem.get_attribute("href")
                if url and "/s/" in url:
                    title_elem = (
                        elem.query_selector("h4")
                        or elem.query_selector("h3")
                        or elem.query_selector("h2")
                    )
                    title = (
                        title_elem.inner_text() if title_elem else f"Article {idx + 1}"
                    )
                    articles.append(WechatArticle(title=title, url=url, index=idx + 1))
        except Exception:
            pass
    ...
    @staticmethod
    def _try_extract_by_list_selector(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        try:
            article_elements = page.query_selector_all("ul.appmsg-list li a")
            for idx, elem in enumerate(article_elements):
                url = elem.get_attribute("href")
                if url and "/s/" in url:
                    title = elem.inner_text() or f"Article {idx + 1}"
    ...
    @staticmethod
    def _try_extract_by_cgi_data(page: Any) -> list[WechatArticle]:
        articles: list[WechatArticle] = []
        ...
        try:
            ...
            if cgi_data and cgi_data.get("continue_flag") == 1:
                articles = WechatParser._fetch_more_articles(page, cgi_data, articles)
        except Exception:
            pass
        ...
        while len(articles) < total_articles and page_count < max_pages:
            if articles and article_list:
                ...
                if last_msgid and last_itemidx:
                    ...
                    page.goto(pagination_url, timeout=30000, wait_until="networkidle")
                    page.wait_for_timeout(3000)

                    updated_cgi_data = page.evaluate("window.cgiData")
