...
from heya.core.temp import TempFileManager
...
class HtmlToPdfConverter:
    ...
    async def convert(
        ...
        try:
            async with self._browser.acquire() as browser:
                pdf_data = await browser.render(source, print_options)
                pdf_content = PdfContent(pdf_data)
...
class MarkdownToPdfConverter:
    ...
    async def convert(
        ...
        with TempFileManager() as temp_manager:
            ...
            return await self._html_converter.convert(
                source=html_url,
                target=target,
                compress=compress,
                quality=quality,
